...
const AES_GCM_OVERHEAD = 48; // salt(32) + tag(16)
...
describe('aesscr CLI', () => {
  should('encrypts and decrypts using AES_PASSWORD', () => {
    ...
    strictEqual(readFileSync(encrypted).length, readFileSync(ref).length + AES_GCM_OVERHEAD);
  ...
  should('prints sha256 checksum only with --sum', () => {
    const plain = join(ROOT, 'sum.txt');
    const encrypted = `${plain}.aesscr`;
    writeFileSync(plain, 'checksum flag\n');
    const sum = createHash('sha256').update(readFileSync(plain)).digest('hex');

    const plainRes = runAesscr(['encrypt', plain], { env: { AES_PASSWORD: PASSWORD } });
    assertOk(plainRes);
    strictEqual(plainRes.stdout.includes('checksum'), false);

    rmSync(encrypted);
    const enc = runAesscr(['encrypt', plain, '--sum'], { env: { AES_PASSWORD: PASSWORD } });
    assertOk(enc);
    strictEqual(enc.stdout.includes(`plaintext sha256 checksum: ${sum}`), true);

    rmSync(plain);
    const dec = runAesscr(['--sum', 'decrypt', encrypted], { env: { AES_PASSWORD: PASSWORD } });
    assertOk(dec);
    strictEqual(dec.stdout.includes(`plaintext sha256 checksum: ${sum}`), true);
  });
